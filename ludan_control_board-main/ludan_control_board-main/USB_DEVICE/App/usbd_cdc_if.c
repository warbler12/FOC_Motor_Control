...
{
  /* USER CODE BEGIN 11 */
  /* Check for motor command frame (new protocol: 0x7C, 16 bytes) */
  if (*Len == CMD_FRAME_LENGTH && Buf[0] == CMD_FRAME_HEADER) {
    /* Motor command frame - put into command queue */
    if (motor_cmd_queue != NULL) {
      osMessageQueuePut(motor_cmd_queue, Buf, 0, 0);
    }
  }
  /* Check for old protocol frame (0x7B, 11 bytes) - backward compatibility */
  else if (*Len == RECEIVE_DATA_SIZE) {
    // num1++;
    ...
    if (rev_data.rx[0] == FRAME_HEADER) // Verify the frame tail of the packet
    {
      ...
      if (rev_data.rx[10] == Check_Sum(10, rev_data.rx)) {
        ...
        if (id < MAX_MOTORS_PER_BUS) {
          dm4340_fbdata_test(&fdcan1_bus.motor[id], rev_data.rx);
        } else {
          dm4340_fbdata_test(&fdcan2_bus.motor[id - MAX_MOTORS_PER_BUS],
                             rev_data.rx);
        }
      } else {
