...
/* USER CODE END MC_Scheduler 0 */
  ...
  if (bMCBootCompleted == 1)                //电机控制系统启动完成
  {
    ...
    {
      TSK_MediumFrequencyTaskM1();          //启用中频任务
...
__weak void TSK_MediumFrequencyTaskM1(void)         
{
  ...
  switch ( StateM1 )
  {
  case IDLE_START:                                              // 启动初始化
    R3_2_TurnOnLowSides( pwmcHandle[M1] );                      // 开启下桥臂，为电容充电
    TSK_SetChargeBootCapDelayM1( CHARGE_BOOT_CAP_TICKS );       // 设置充电延时
    STM_NextState( &STM[M1], CHARGE_BOOT_CAP );
    break;

  case CHARGE_BOOT_CAP:
    if ( TSK_ChargeBootCapDelayHasElapsedM1() )                 // 检查充电时间是否完成
    {
      PWMC_CurrentReadingCalibr( pwmcHandle[M1], CRC_START );   // 开始电流采样校准
