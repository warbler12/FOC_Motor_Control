...
void rt_OneStep(void)
{
   ...
   */
  mcb_pmsm_qep_offset_f28379d_SetEventsForThisBaseStep(eventFlags);
  enableTimer0Interrupt();
  mcb_pmsm_qep_offset_f28379d_step0();
  ...
  if (need2runFlags[1]) {
    ...
    {
     case 1 :
      mcb_pmsm_qep_offset_f28379d_step1();
...
int main(void)
{
  ...
  runModel = rtmGetErrorStatus(mcb_pmsm_qep_offset_f28379d_M) == (NULL);
  enableTimer0Interrupt();
  globalInterruptEnable();
  while (runModel) {
    stopRequested = !(rtmGetErrorStatus(mcb_pmsm_qep_offset_f28379d_M) == (NULL));
  }
