...
pmsm.Ld = 0.00086;       % D轴电感 (H) 
pmsm.Lq = 0.00086;       % Q轴电感 (H) - 假设Ld = Lq
pmsm.FluxPM = 0.0035;    % 永磁体磁链 (Wb) - 计算得出，见下方说明
pmsm.p = 5;             % 极对数
pmsm.I_rated = 4;      % 额定电流 (A) - 峰值
pmsm.Ke = 0.0434;
pmsm.Kt = pmsm.Ke;
pmsm.N_rated = 200;     % 额定转速 (RPM)
pmsm.N_max = 260;     % 额定转速 (RPM)
pmsm.Vdc = 12.0;         % 直流母线电压 (V)
pmsm.mechanical_offset = 0.65; 
%pmsm.mechanical_offset = 0.75;      %零点校准 PU


% 可选参数
pmsm.J = 3.7e-6;           % 转动惯量 (kg·m²) - 估计值，需要调整
pmsm.B = 1e-6;           % 摩擦系数 - 估计值
...
inverter.V_dc = 12;
inverter.CtSensAOffset = 2278;
inverter.CtSensBOffset = 2256;
...
PI_params.Ki_speed = 0.15;
PI_params.Kp_pos = 0.4;
PI_params.Ki_pos = 0;
