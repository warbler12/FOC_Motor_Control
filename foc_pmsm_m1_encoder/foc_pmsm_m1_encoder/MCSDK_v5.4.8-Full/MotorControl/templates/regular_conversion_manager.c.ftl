...
<#assign CondFamily_STM32F1 = (FamilyName?? && FamilyName=="STM32F1") >
...
/**
  ...
  */
#define RCM_MAX_CONV <#if MC.SINGLEDRIVE > 4 <#else> 6 </#if>
  ...
  uint8_t i=0;
    ...
    {    
      ...
      if (LL_ADC_IsEnabled(regConv->regADC) == 0 )
      {
       ...
       <#elseif  CondFamily_STM32F3 || CondFamily_STM32L4 || CondFamily_STM32G4>
       ...
       <#if CondFamily_STM32F1 >
        ...
        <#if !CondFamily_STM32F4 && !CondFamily_STM32L4 && !CondFamily_STM32F7>
         <#if CondFamily_STM32F0 || CondFamily_STM32G0 || CondFamily_STM32F1>        
        ...
        */
        ...
        <#elseif CondFamily_STM32F1 >
        <#-- Nothing to do STM32F1 must be enable before calibration -->
        <#else>
        LL_ADC_Enable( regConv->regADC );
        </#if>
...
<#if NoInjectedChannel>
  ...
  <#else>
  if ((PWM_Handle_M1.ADCRegularLocked == false) && (PWM_Handle_M2.ADCRegularLocked == false))
   </#if>
